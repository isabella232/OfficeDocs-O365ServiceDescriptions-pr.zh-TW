...
| 敏感度標籤                    | 自動分類及標籤 for Office app (Word、Excel、PowerPoint、Outlook) 跨平臺 (網頁、Android、iOS、Windows 及 Mac)  |  可以使用              |
| 敏感度標籤                    | 自動分類及標記 Office 用戶端 (行動)                                        | 在工程處理積壓工作              |
| 敏感度標籤                    | 針對團隊、Microsoft 365 群組、SharePoint 網站的自動分類和標籤                            | 可以使用 |
...
| 資訊控管                 | 電子郵件封存          | 可以使用              |
| 資訊控管                 | SharePoint 的預設保留標籤、商務文件庫、資料夾及檔集的 OneDrive;Exchange 收件匣;和 Office 365 群組          | 可以使用              |
| 資訊控管                 | 匯入 PST                      | 可以使用              |
| 資訊控管                 | 手動非記錄保留標籤            | 可以使用 |
| 資訊控管                 | 保留鎖定            | 可以使用 |
| 資訊控管                 | 保留原則給整個組織;特定位置或使用者。根據特定條件自動根據 (例如，關鍵字或機密資訊) ;根據事件                                       | 可以使用              |
| 資訊控管                 | 小組的保留原則                            | 可以使用 |
| 資訊控管                 | 小組會議錄製的保留原則                            | 在開發 |
| 資訊控管                 | 小組專用通道的保留原則                            | 在工程處理積壓工作 |
| 資訊控管                 | 小組共用通道的保留原則                            | 在工程處理積壓工作 |
...
| 記錄管理                     | 適用于記錄管理的 Outlook 用戶端支援    | 在工程處理積壓工作 |
...
| 電子文件探索                              | 核心 eDiscovery： Microsoft 規範中心擴充支援，可在 SharePoint 和 OneDrive 商務用回收站中搜尋及匯出專案                                        | 可以使用              |
...
| 電子文件探索                              | Advanced eDiscovery： Export (下載、匯出、新增至另一個 view set)                                           | 可以使用 |
...
| 稽核                                   | 高級審核： Microsoft 365 安全性與合規性中心                    | 可以使用 |
