...
| | 自動分類及標籤，用於 Office 應用程式 (Word、Excel、PowerPoint、Outlook 跨平臺) 網頁、 (和 Mac Windows | 可以使用 |
| | Office 用戶端 (行動) 的自動分類和標籤 | 在工程處理積壓工作 |
| | Teams、Microsoft 365 群組、SharePoint 網站的自動分類和標籤 | 可以使用 |
...
| | Microsoft 365 多工作負載加密的客戶金鑰 | 可以使用 |
...
| | 記錄管理：透過 trainable 的分類器自動套用記錄原則 | 在開發 |
| | 記錄管理：處置評審 | 可以使用 |
| | 記錄管理：檔案計畫管理員 | 可以使用 |
| | 記錄管理：多階段處置評審 | 在開發 |
| | 記錄管理：針對記錄管理 Outlook 用戶端支援 | 在開發 |
...
| | 通訊相容性： Teams 交談內容 | 在開發 |
| | 通訊相容性：在調查期間翻譯內容 | 可以使用 |
| 資訊障礙 | 資訊障礙 | 可以使用 |
...
| | 核心 eDiscovery：商務用 OneDrive 的規範界限 | 可以使用 |
| | 核心 eDiscovery：匯出 | 可以使用 |
| | 核心 eDiscovery：就地保留 | 可以使用 |
| | 核心 eDiscovery：原生匯出 | 可以使用 |
| | 核心 eDiscovery： RMS 解密 | 可以使用 |
| | 核心電子檔探索：搜尋 | 可以使用 |
| | 核心 eDiscovery： Microsoft 規範中心擴充支援，可搜尋和匯出 SharePoint 和商務用 OneDrive 回收站中的專案 | 可以使用 |
| | Advanced eDiscovery：高級處理 | 可以使用 |
| | Advanced eDiscovery： Custodial 至工作負載對應 | 可以使用 |
...
| | Advanced eDiscovery：雙位元組字元支援 (中文、日文、韓文)  | 可以使用 |
| | Advanced eDiscovery：電子郵件執行緒 | 可以使用 |
| | Advanced eDiscovery：匯出 (下載、匯出、新增至另一個 view set)  | 可以使用 |
| | Advanced eDiscovery：篩選 | 可以使用 |
| | Advanced eDiscovery：保留優化 | 在開發 |
| | Advanced eDiscovery： Teams 私人通道郵件的法律封存 | 可以使用 |
...
| | 高級審計： Microsoft 365 安全性與合規性中心 | 可以使用 |
...
| |  (17a-4 和 CellTrust 連接器的協力廠商資料連線器)  | 在開發 |
| | 協力廠商資料連線器 (Telemessage)  | 可以使用 |
| |  (Veritas) 的協力廠商資料連線器 | 逐步推出 |
| |  (17a-4 和 CellTrust 連接器的協力廠商資料連線器)  | 在開發 |
