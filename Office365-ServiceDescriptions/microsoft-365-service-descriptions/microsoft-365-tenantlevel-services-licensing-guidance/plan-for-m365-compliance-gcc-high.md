...
為了符合我們政府雲端客戶的需求，Microsoft 365 政府– GCC-High 和企業方案之間有一些差異。 請參閱下表，以查看哪些功能可供使用。
...
|                                         | 雙重金鑰加密                           | 在開發 |
|                                         | 使用 WXP web app 在加密檔上共同撰寫         | 在工程處理積壓工作 |
|                                         | 檔和電子郵件的資料遺失防護         | 可以使用 |
...
|                                         | 通訊相容性 (包括。監察原則) ：建立客戶原則，3預先設定  | 在開發 |
|                                         | 通訊相容性 (包括。監察原則) ：對團隊、Exchange 及移除小組郵件的支援 | 在開發 |
...
|                                         | Advanced eDiscovery： Export (下載、匯出、新增至另一個評審集)                    | 可以使用 |
...
|                                         | Advanced eDiscovery： Graph APIs                              | 在開發 |
|                                         | Advanced eDiscovery：支援小組反應                             | 在開發 |
