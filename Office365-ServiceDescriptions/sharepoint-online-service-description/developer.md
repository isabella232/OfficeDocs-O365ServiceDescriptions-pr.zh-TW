...
## <a name="app-catalog-sharepoint"></a>應用程式目錄 (SharePoint)
...
## <a name="bcs-odata-connector"></a>BCS：OData 連接器
...
## <a name="client-object-model-om"></a>用戶端物件模型 (OM)
...
使用 Office 365 開發人員網站作為開發和測試環境，以縮短設定時間並開始建立、 測試和部署 SharePoint 應用程式。 深入了解[設定適用於 Office 365 開發人員網站簽署](https://docs.microsoft.com/sharepoint/dev/sp-add-ins/create-a-developer-site-on-an-existing-office-365-subscription)。
  
## <a name="forms-based-applications"></a>表單型應用程式

表單檢視基本上是包含控制項的檢視。 表單型應用程式可讓使用者建立，並使用應用程式內的一或多個表單。 深入了解[表單型應用程式](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-6.0/aa733955(v=vs.60))。
  
## <a name="full-trust-solutions"></a>完全信任解決方案

不適用於 SharePoint Online 客戶。 SharePoint Server 2013 客戶可以建立完全信任解決方案 （也稱為伺服器陣列方案）。 不同 SharePoint 應用程式，伺服器陣列解決方案包含程式碼，就會部署至 SharePoint 伺服器，並讓 SharePoint 的伺服器物件模型呼叫。 這些組件一律執行以完全信任。 伺服器陣列解決方案應用於自訂的 SharePoint 系統管理職責，例如自訂計時器工作、 自訂的 Windows PowerShell cmdlet 及副檔名的管理中心。 深入了解[建置 SharePoint 2013 中的伺服器陣列解決方案](https://docs.microsoft.com/sharepoint/dev/general-development/build-farm-solutions-in-sharepoint)。
  
## <a name="infopath-forms-services"></a>InfoPath Forms Services

表單服務可在 SharePoint 中，根據 InfoPath 中設計的表單範本的網頁瀏覽器填寫表單體驗。 進一步了解 [InfoPath Forms Services。](https://docs.microsoft.com/previous-versions/office/developer/sharepoint-2007/ms540731(v=office.12))
  
## <a name="javascript-object-model"></a>JavaScript 物件模型

SharePoint 提供 JavaScript 物件模型，可用於內嵌指令碼或獨立 .js 檔案。 它包含與 .NET Framework 和 Silverlight 用戶端物件模型相同的所有功能。 JavaScript 物件模型是將自訂 SharePoint 程式碼加入應用程式的有用方法。 它也可讓網頁開發人員使用其現有的 JavaScript 技能建立 SharePoint 應用程式具有最少的學習曲線。 進一步了解 [SharePoint 2013 的 JavaScript API 參照](https://docs.microsoft.com/previous-versions/office/sharepoint-visio/jj193034(v=office.15))。
  
## <a name="remote-event-receiver"></a>遠端事件接收器
...
使用 SharePoint Designer，進階使用者和開發人員可以快速地建立 SharePoint 解決方案以回應的業務需求。 進一步了解[適用於開發人員的 SharePoint Designer](https://go.microsoft.com/fwlink/?LinkId=271294)。
