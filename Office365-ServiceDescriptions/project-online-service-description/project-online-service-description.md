...
Web 的專案是以 Microsoft Power 平臺為基礎。 電源平臺包括 PowerApps、電源自動化、Power BI 及一般資料服務（CD）。 Web 資料的專案儲存在 CD 中。 
...
在上述表格中，檢查數目（**x**）表示方案所提供的一般功能層級。 例如，針對 Project Online，方案5提供更多功能，所以方案3。
...
|--------|-------------|
|**專案、任務和時間管理**||
|板視圖 | 使用現成和自訂的任務板，以視覺方式追蹤專案工作，以改進工作流程和狀態監控。|
|相依性 | 設定並以視覺方式追蹤任務間的相依性。|
|格線視圖 | 使用任務的階層式格線視圖，規劃及管理專案。|
|里程碑 | 建立及追蹤重要的專案和任務里程碑。|
|專案首頁 | 取得最近使用之專案的鳥瞰視圖，以及您需要密切監視的重要專案。|
|任務排程 | 使用任務的開始和結束日期、工作、工作和相依性，以取得專案日期的準確排程。|
|時序表視圖（甘特圖） | 在階層式時程表視圖中，以視覺方式追蹤和瞭解專案日期、相依性和工作分派。|
|摘要任務 | 使用摘要工作規劃、管理及追蹤可交付作業與階段。|
|**共同作業**||
|Microsoft 小組整合 | 針對 Microsoft 團隊中的專案進行共同作業。<sup>1</sup>|
|**資源和程式管理**||
|專案小組設定 | 新增及移除成員，並設定工作行事曆，以管理專案小組。|
|藍圖 | 從 Microsoft Project Online、Microsoft Project for web 和 Azure 董事會專案，建立跨專案的視覺、互動的路標。<sup>2</sup>使用專案計劃1您可以取得對路標的唯讀許可權。 使用專案計劃3或專案方案5，您也可以建立路標。 |
|**報告**<sup>3</sup>||
|開箱即用報告 | 使用預先建立的報告，以瞭解專案、程式、產品群組和資源資料。|
|自訂報告 | 根據現成報表或從頭開始建立自訂報表，瞭解組織的重要資料。|
|儀表板及入口網站 | 建立自訂儀表板及入口網站，以協助滿足特定組織的需求。|
|**可用 性**||
|Coauthoring | 與專案關係人和小組成員合作，以同時建立、編輯和更新工作清單、專案排程等等。|
|圖形標記 | 瞭解具有圖形標記的任務狀態、工作分派及其他資訊。 |
|**行動性**||
|行動應用程式 | 使用 PowerApps 建立自訂行動應用程式。<sup>4</sup> |
|**自訂和整合**||
|PowerApps | 使用 PowerApps 建立及使用 web 和行動裝置的解決方案。<sup>4</sup> |
|工作流程 | 自動化和整合商務程式和電源自動化功能。<sup>5</sup> |
|Microsoft 整合 | 共同作業、管理檔，以及使用熟悉的 Microsoft 工具和應用程式。<sup>1</sup> |
|**安全性和使用者管理**||
|Office 現代化群組 | 組建專案小組以共同作業，並輕鬆地設定小組成員共用的資源集合。|
...
|資源預訂（預定） | 專案經理可以依角色、技能或名稱提交資源的要求。 資源管理員可以查看要求並指派可供使用的最佳資源。 |是<sup>7</sup> | 是<sup>8</sup> |
...
|產品群組分析和優先順序 | 識別、選取及交付最適合組織之商務策略的專案組合，並最大化投資回報率（ROI）。 | 否 | 是 |
...
|PowerApps | 使用 PowerApps 建立及使用 web 和行動的解決方案（透過 OData）。<sup>4</sup> | 是 | 是 |
|工作流程 | 使用自動功能自動化和整合商務程式（透過 OData）。<sup>5</sup> | 是 | 是 |
...
|**安全性、使用者和服務管理**||||
|Active Directory 整合 | 從組織的 Active Directory 的任何成員建立小組。<sup>9</sup> | 是 | 是 |
...
<sup>8</sup>具有專案方案5訂閱的使用者可以查看、履行和核准資源預訂要求。</br>
...
|**工作** ||
|麥克斯。 任務的階層層級 | 10層級 |
|麥克斯。 工作的連結（後續的 + 前置任務） | 共 |
|麥克斯。 葉任務的工期 | 1250天 |
|麥克斯。 摘要任務的工期 | 3650天（10年） |
|麥克斯。 可指派給任務的資源 | 20個資源 |
|任務的支援日期範圍 | 1/1/1984 &ndash; 12/31/2149 |
|**Project** ||
|麥克斯。 專案的任務總數 | 500 |
|麥克斯。 專案的總持續時間 | 3650天（10年） |
|麥克斯。 專案的總資源 | 150 |
|麥克斯。 專案的總連結（僅限後續任務） | 600 |
