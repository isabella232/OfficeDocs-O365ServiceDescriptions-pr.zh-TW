...
|時序表視圖（甘特圖） | 在階層式時程表視圖中，以視覺方式追蹤和瞭解專案日期、相依性和工作分派。|僅檢視 | 是 | 是 | 是 |
...
|藍圖 | 從 Microsoft Project Online、Microsoft Project for web 和 Azure 董事會專案，建立跨專案的視覺、互動的路標。<sup>2</sup>  使用專案計劃1時，您會取得對路標的唯讀存取權。 使用專案計劃3或專案方案5，您也可以建立路標。 | 僅檢視 | 僅檢視 | 是 | 是 |
|**報告**<sup>3</sup> ||||||
|開箱即用報告 | 使用預先建立的報告，以瞭解專案、程式、產品群組和資源資料。 | 否 | 是 | 是 | 是 |
|自訂報告 | 根據現成報表或從頭開始建立自訂報表，瞭解對組織重要的資料。 | 否 | 是 | 是 | 是 |
|儀表板及入口網站 | 建立自訂儀表板及入口網站，以協助滿足特定組織的需求。| 否 | 是 | 是 | 是 |
|**可用 性** ||||||
...
|**自訂和整合**||||||
|PowerApps | 使用 PowerApps 建立及使用 web 和行動裝置的解決方案。<sup>4</sup> | 否 | 否 | 是 | 是 |
|工作流程 | 自動化和整合商務程式和電源自動化功能。<sup>5</sup> | 否 | 否 | 是 | 是 |
|Microsoft 整合 | 使用熟悉的 Microsoft 工具和應用程式共同作業、管理檔等。<sup>1</sup> | 是 | 是 | 是 | 是 |
|**安全性和使用者管理**||||||
...
<sup>3</sup>使用 power BI 可取得 Web 和 project Online 的專案。 查看、編輯及建立報表需要 Power BI 訂閱。 Microsoft 提供一組預先構建的 Power BI 報告。 使用者可以使用 Power BI 自訂這些報告或建立其他報告。</br> 
