...
此功能適用于網頁 Excel。 其他的高級功能僅適用于 Excel 桌面應用程式。
...
在儲存格中輸入的 Excel 可以完成您正在輸入儲存格的內容，如果有一個以上的可能值，請在您可以選擇的清單中顯示所有值。
...
您可以在 Excel 中插入、編輯和刪除網頁的批註。 您也可以解析批註執行緒，它會將它關閉以進行編輯及新增至新的批註，但會將它保留在原地。 這可讓您視需要將螺紋重新開啟為批註。 [在您的批註中使用 @mentions](https://support.office.com/article/644bf689-31a0-4977-a4fb-afe01820c1fd?ad=US#ID0EAADAAA=Online) ，將電子郵件傳送給您提及的人員。 進一步了解[在 Excel 中新增註解](https://go.microsoft.com/fwlink/p/?LinkId=271670)。
...
您可以剪下、複製及貼上文字、超連結、數位、公式、圖形、圖表及影像。 [！注意] 如果您的資料列中有您必須旋轉以重新排列資料列的資料，請使用 [**貼**上] 功能表上的 [**換位**] 功能。 深入瞭解[在 Excel 中複製及貼上網頁](https://support.office.com/article/682704da-8360-464c-9a26-ff44abf4c4fe#bmk-excelonline)。
  
## <a name="create-external-data-connections"></a>建立外部資料連結

您可以使用 Excel 來查看資料連線，但無法使用網頁版 Excel 建立外部資料連線。 您需要 Excel 桌面應用程式才能使用外部資料。 進一步了解[在 Excel Services 中使用外部資料](https://go.microsoft.com/fwlink/p/?LinkId=282330)。 
  
## <a name="create-tables"></a>建立表格

建立表格以組織和分析相關資料。表格便於進行排序、篩選和格式化資料。 
  
## <a name="creation-of-advanced-analysis-views-power-view-power-pivot-slicers"></a>建立進階分析檢視 (Power View、Power Pivot、交叉分析篩選器)

僅 Excel 桌面應用程式提供進階分析檢視 (例如：Power Pivot 和 Power View) 功能。 網頁版 Excel 不支援這些功能。 進一步了解 [PowerPivot](https://go.microsoft.com/fwlink/p/?LinkId=271671)。
...
您可以將儲存格從一個位置拖放到另一個位置，以在 Excel 中移動儲存格的值。
...
外部參考 (也稱為連結) 是其他 Excel 活頁簿中工作表上的儲存格或範圍的參考，或是另一個活頁簿中所定義名稱的參考。 您可以使用 Excel 來查看外部參照，但是無法使用 Excel 來建立網頁。 您將需要 Excel 桌面應用程式，才能建立或更新外部參照。 
...
請參閱資料編輯列中儲存格結果後方的公式。 您可以在瀏覽器中新增、變更和刪除部分公式，就如同在 Excel 桌面應用程式中操作一樣。 Excel 網頁會以色彩顯示公式的引數，在公式與工作表上的資料之間提供視覺對應。
  
## <a name="formula-tools-advanced"></a>公式工具，高級

Microsoft Excel 桌面應用程式支援最進階的公式工具，例如：3D 參照樣式和陣列公式。 僅限網頁版 excel 支援最基本的 Excel 公式。 進一步了解[公式](https://go.microsoft.com/fwlink/p/?LinkId=271665)。
...
在網頁版 Excel 中，您可以在公式中使用大部分的 Excel 400 以上的工作表函數。
...
您可以隱藏和取消隱藏網頁版 Excel 活頁簿中的列、欄和工作表。
...
您無法在 Excel 中為 web 建立命名範圍，但是您可以在網頁型 excel 的試算表中，使用您在 Excel 桌面中所建立的命名範圍。 當您選取一個命名範圍時，該名稱會顯示在公式欄的上方。
...
## <a name="office-add-ins"></a>Office 增益集
 ...
 在 Excel 中，有些（並非所有的 Office 增益集）是可用於 web 的。 您可以從 Office 市集取得 Excel 增益集。 進一步了解[如何取得 Excel 增益集](https://go.microsoft.com/fwlink/p/?LinkId=271667)。

## <a name="offline-viewing-and-authoring"></a>離線檢視和撰寫

Web 的 Excel 從網際網路瀏覽器啟動，並依賴網際網路連線。 若要離線存取試算表，您必須在電腦上安裝 Microsoft Excel 桌面應用程式，以用來檢視和編輯 Excel 試算表。
...
您可以將所選的試算表內容嵌入社交網路頁面或商務用 Skype 對話或會議中，即可和其他使用者共用選取的部分。 網頁版 Excel 不支援這些功能。
  
## <a name="power-pivot-viewing"></a>Power Pivot 檢視

網頁版 excel 可讓您查看 Power Pivot 表格和圖表，但是您需要 Excel 桌面應用程式才能建立 Power Pivot 資料模型。
  
## <a name="power-view-viewing"></a>Power View 檢視

您可以在活頁簿中查看 Power View 工作表並與其互動，但無法使用網頁版 Excel 建立 Power View。 只有使用桌面應用程式才能建立 Power View。 Power View 可讓您在互動式、能隨時呈現的檢視中，建立、共用及瀏覽資料。 在單一工作表上相互連結的互動式地圖、圖表和表格中檢視資訊。 進一步了解 [Power View](https://go.microsoft.com/fwlink/p/?LinkId=271674)。
...
僅 Microsoft Excel 桌面應用程式支援進階圖表功能。 網頁版 Excel 不支援這些功能。 進一步了解[可用的圖表類型](https://go.microsoft.com/fwlink/p/?LinkId=271666)。
  
## <a name="refresh-existing-data-connections"></a>重新整理現有的資料連接

如果活頁簿中的資料連線至外部來源（例如資料庫、網頁或分析 cube），則可以使用網頁版 Excel 來重新整理該資料。
...
在網頁版的 Excel 中，您可以在不關閉活頁簿的情況下重新命名活頁簿，方法是選取標頭中的檔案名，然後輸入新的名稱。 若要使用 Excel 桌面應用程式重新命名檔案，則需要關閉檔案、瀏覽至其在裝置上的位置，然後重新命名檔案。
...
## <a name="rights-management-irm-and-password-security"></a>版權管理：IRM 和密碼安全性
 ...
 在 Excel 桌面應用程式中，您可以使用密碼、權限和其他限制方式保護活頁簿的資料安全。 在 web 的 Excel 中，您可以載入包含工作表保護的活頁簿並與其互動，以防止使用者選取或輸入受保護的儲存格。 進一步了解[保護 Excel 活頁簿的資料安全](https://go.microsoft.com/fwlink/p/?LinkId=271672)。
...
## <a name="slicer-viewing"></a>交叉分析篩選器檢視

在工作表中查看交叉分析篩選器，並選取 [交叉分析篩選] 按鈕以篩選資料。 您無法在 Excel for web 中建立或編輯切片器。
  
## <a name="sort-and-filter-data"></a>排序和篩選資料

在 web 的 Excel 中，您可以查看和重新排序所有工作表、排序及篩選資料（自上而下或從左至右，多個層級），然後深入瞭解 PivotTables 的詳細資料。 深入瞭解[排序和篩選資料](https://support.office.com/article/62d0b95d-2a90-4610-a6ae-2e545c4a4654?ad=US#ID0EAADAAA=Web)。
  
## <a name="spreadsheet-audit-and-compliance"></a>試算表審核和法規遵循

在 Microsoft Excel 桌面應用程式中，您可以使用工作表的查詢和比較功能來比較活頁簿版本、分析活頁簿的問題或不一致情況，或查看活頁簿和工作表之間的連結。 網頁版 Excel 不支援這些功能。 進一步了解[如何使用試算表查詢功能](https://go.microsoft.com/fwlink/p/?LinkId=271677)。
  
## <a name="status-bar-aggregates"></a>狀態列彙總

當您在 Excel 中為 web 選取一組儲存格時，您可以在狀態列中看到 SUM、AVERAGE 和 COUNT。 您也可以選擇想要查看的彙總來自訂狀態列。
...
當您需要在 Excel 中為網頁完成某些作業，但是不知道如何執行時，您可以使用「告訴我搜尋」搜尋功能，快速找出您要尋找的專案。 「操作說明搜尋」了解您正在嘗試完成的作業，並且會提供建議，協助您更快速地完成作業。

## <a name="text-formatting"></a>文字格式

您可以套用各種文字格式設定選項，包括刪除線、增加/減少網頁版 Excel 中的縮排。
...
在您的工作表中檢視 3D 圖表。 您無法在 Excel 中為 web 建立或編輯3D 圖表。
  
## <a name="what-if-analysis-tools"></a>What if 分析工具

無法用於 web 的 Excel。 如預期所示，您可以在 Excel 中查看「假設」分析的結果。 如果您想使用分析工具 (例如：目標搜尋、資料表、規劃求解和數列)，則您需要 Excel 桌面應用程式。 進一步了解[在瀏覽器中和 Excel 桌面應用程式中使用活頁簿的差異](https://go.microsoft.com/fwlink/p/?LinkId=273900)。
